...
void main() {
    if( picking ) {
        ...
        vec3 I = normalize(fs_in.position_ES - cameraPos);
        vec3 R = reflect(I, normalize(fs_in.normal_ES));
        vec4 reflectedColour = vec4(texture(skybox, R).rgb, 1.0);
        fragColour = vec4(phongModel(fs_in.position_ES, fs_in.normal_ES), 1.0);
        fragColour = mix(fragColour, reflectedColour, 0.4);
    }
}
